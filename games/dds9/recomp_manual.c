...
#include <stdio.h>
#include <stdint.h>
#include <stddef.h>   /* ptrdiff_t, for g_xbox_mem_offset below */
...
/* Must match recomp_types.h and xbox_memory_layout.h. The register file is
 * thread-local, and a plain `extern uint32_t g_eax;` against a __thread
 * definition does not resolve to the calling thread's copy -- it reads the
 * image's TLS template, so every value printed below would be a constant zero
 * rather than the faulting thread's state. */
#if defined(_MSC_VER)
...
{
    /* Name the call site, not just the target.
     *
     * RECOMP_ICALL_SAFE calls this before it restores g_esp, and the generated
     * code pushes the guest return address immediately before invoking the
     * macro -- so the top of the guest stack still holds the address of the
     * instruction after the call. That is the one fact that turns "some
     * indirect call failed" into a location that can be looked up in the
     * disassembly and in recomp_funcs.h. */
    uint32_t site = 0;
    if (g_xbox_mem_offset && g_esp)
        site = *(volatile uint32_t *)((uintptr_t)g_xbox_mem_offset + g_esp);

    fprintf(stderr, "[ICALL] Failed to resolve VA 0x%08X from call site 0x%08X "
                    "(total calls: %llu)\n",
            va, site, (unsigned long long)g_icall_count);
